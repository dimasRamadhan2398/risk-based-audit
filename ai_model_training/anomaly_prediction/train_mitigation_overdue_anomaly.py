...
dataset_path = os.path.join(current_dir, 'mitigation_overdue_data.csv')
print(f"Loading Mitigation Overdue dataset from: {dataset_path}")
...
# ======================================================================
print("\n======================================================================")
print(" 1. MITIGATION OVERDUE DATA PREPROCESSING & FEATURE ENGINEERING ")
print("======================================================================")

# Step 1: Strip surrounding spaces
text_columns = [col for col in ['Entitas Auditee', 'Kategori Temuan', 'Tingkat Keparahan Temuan', 'Deskripsi Rencana Aksi', 'TARGET: is_anomaly'] if col in df.columns]
df[text_columns] = df[text_columns].apply(lambda col: col.astype("string").str.strip())

# Step 2: Convert numeric columns
numeric_cols = ['overdue_days', 'repeated_overdue_count', 'mitigation_progress_pct', 'is_unjustified_delay (1=Ya, 0=Tidak)']
df[numeric_cols] = df[numeric_cols].apply(pd.to_numeric, errors='coerce')

# Step 3: Deduplication
duplicate_cols = [col for col in df.columns if col != 'ID Mitigasi']
...
# Step 4: Advanced Mitigation Domain Feature Engineering
print("Applying Domain-Specific Action Plan Feature Engineering...")
df['log_overdue_days'] = np.log1p(df['overdue_days'])
df['delay_severity_index'] = df['overdue_days'] * (df['repeated_overdue_count'] + 1)
df['progress_lag_gap'] = (100.0 - df['mitigation_progress_pct']) * (df['overdue_days'] + 1)
df['risk_composite_score'] = (df['is_unjustified_delay (1=Ya, 0=Tidak)'] * 3.0) + \
                             ((df['overdue_days'] / 30.0) * 1.5) + \
                             (df['repeated_overdue_count'] * 0.8)

# Input Feature Columns List
feature_cols = [
    'Entitas Auditee', 'Kategori Temuan', 'Tingkat Keparahan Temuan', 'Deskripsi Rencana Aksi',
    'overdue_days', 'repeated_overdue_count', 'mitigation_progress_pct', 
    'is_unjustified_delay (1=Ya, 0=Tidak)', 'log_overdue_days', 
    'delay_severity_index', 'progress_lag_gap', 'risk_composite_score'
]

X_raw = df[feature_cols].values
print(f"Selected {len(feature_cols)} clean mitigation input features.")

# One-Hot Encoding for categorical columns (0: Entitas Auditee, 1: Kategori Temuan, 2: Tingkat Keparahan Temuan, 3: Deskripsi Rencana Aksi)
...
targets_config = {
    'Mitigation Overdue Anomaly Detection (is_anomaly)': {
        'column': 'TARGET: is_anomaly',
        'transform': lambda y: np.where(y == 'Ya (Anomali)', 1, 0),
        'class_names': ['Normal Mitigation', 'Anomaly Mitigation Overdue'],
        'is_binary': True
    },
