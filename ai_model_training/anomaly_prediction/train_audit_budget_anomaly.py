...
dataset_path = os.path.join(current_dir, 'audit_budget_data.csv')
print(f"Loading Audit Budget dataset from: {dataset_path}")
...
# ======================================================================
print("\n======================================================================")
print(" 1. AUDIT BUDGET DATA PREPROCESSING & FEATURE ENGINEERING ")
print("======================================================================")

# Step 1: Strip surrounding spaces
text_columns = [col for col in ['Entitas Auditee', 'Kategori Perencanaan', 'Deskripsi Perencanaan', 'TARGET: is_anomaly'] if col in df.columns]
df[text_columns] = df[text_columns].apply(lambda col: col.astype("string").str.strip())

# Step 2: Convert numeric columns
numeric_cols = [
    'allocated_budget_juta', 'historical_avg_budget_juta', 
    'allocated_mandays', 'assigned_auditors_count', 
    'budget_zscore', 'is_unapproved_budget_spike (1=Ya, 0=Tidak)'
]
df[numeric_cols] = df[numeric_cols].apply(pd.to_numeric, errors='coerce')

# Step 3: Deduplication
duplicate_cols = [col for col in df.columns if col != 'ID Rencana Audit']
...
# Step 4: Advanced Audit Budget & Resource Feature Engineering
print("Applying Domain-Specific Audit Budget Feature Engineering...")
df['log_allocated_budget'] = np.log1p(df['allocated_budget_juta'])
df['budget_per_manday'] = df['allocated_budget_juta'] / (df['allocated_mandays'] + 0.1)
df['mandays_per_auditor'] = df['allocated_mandays'] / (df['assigned_auditors_count'] + 0.1)
df['risk_composite_score'] = (df['is_unapproved_budget_spike (1=Ya, 0=Tidak)'] * 3.0) + \
                             (np.abs(df['budget_zscore']) * 1.5) + \
                             (df['budget_per_manday'] * 0.2)

# Input Feature Columns List
feature_cols = [
    'Entitas Auditee', 'Kategori Perencanaan', 'Deskripsi Perencanaan',
    'allocated_budget_juta', 'historical_avg_budget_juta', 
    'allocated_mandays', 'assigned_auditors_count', 
    'budget_zscore', 'is_unapproved_budget_spike (1=Ya, 0=Tidak)',
    'log_allocated_budget', 'budget_per_manday', 'mandays_per_auditor', 'risk_composite_score'
]

X_raw = df[feature_cols].values
print(f"Selected {len(feature_cols)} clean audit budget input features.")

# One-Hot Encoding for categorical columns (0: Entitas Auditee, 1: Kategori Perencanaan, 2: Deskripsi Perencanaan)
...
targets_config = {
    'Audit Budget Anomaly Detection (is_anomaly)': {
        'column': 'TARGET: is_anomaly',
        'transform': lambda y: np.where(y == 'Ya (Anomali)', 1, 0),
        'class_names': ['Normal Budget', 'Anomaly Budget'],
        'is_binary': True
    },
