...
dataset_path = os.path.join(current_dir, 'risk_score_spike_data.csv')
print(f"Loading Risk Score Spike dataset from: {dataset_path}")
...
# ======================================================================
print("\n======================================================================")
print(" 1. RISK SCORE SPIKE DATA PREPROCESSING & FEATURE ENGINEERING ")
print("======================================================================")

# Step 1: Strip surrounding spaces
text_columns = [col for col in ['Entitas Auditee', 'Kategori Risiko', 'Deskripsi Evaluasi Risiko', 'TARGET: is_anomaly'] if col in df.columns]
df[text_columns] = df[text_columns].apply(lambda col: col.astype("string").str.strip())

# Step 2: Convert numeric columns
numeric_cols = [
    'current_risk_score', 'previous_risk_score', 'risk_score_diff', 
    'new_findings_count', 'risk_score_volatility', 'is_unexplained_spike (1=Ya, 0=Tidak)'
]
df[numeric_cols] = df[numeric_cols].apply(pd.to_numeric, errors='coerce')

# Step 3: Deduplication
duplicate_cols = [col for col in df.columns if col != 'ID Penilaian Risiko']
...
# Step 4: Advanced Risk Score Spike Feature Engineering
print("Applying Domain-Specific Risk Assessment Feature Engineering...")
df['spike_ratio'] = df['current_risk_score'] / (df['previous_risk_score'] + 1e-5)
df['findings_spike_ratio'] = df['risk_score_diff'] / (df['new_findings_count'] + 0.1)
df['risk_composite_score'] = (df['is_unexplained_spike (1=Ya, 0=Tidak)'] * 3.0) + \
                             (df['risk_score_diff'] * 1.5) + \
                             (df['risk_score_volatility'] * 1.2)

# Input Feature Columns List
feature_cols = [
    'Entitas Auditee', 'Kategori Risiko', 'Deskripsi Evaluasi Risiko',
    'current_risk_score', 'previous_risk_score', 'risk_score_diff', 
    'new_findings_count', 'risk_score_volatility', 'is_unexplained_spike (1=Ya, 0=Tidak)',
    'spike_ratio', 'findings_spike_ratio', 'risk_composite_score'
]

X_raw = df[feature_cols].values
print(f"Selected {len(feature_cols)} clean risk score spike input features.")

# One-Hot Encoding for categorical columns (0: Entitas Auditee, 1: Kategori Risiko, 2: Deskripsi Evaluasi Risiko)
...
targets_config = {
    'Risk Score Spike Anomaly Detection (is_anomaly)': {
        'column': 'TARGET: is_anomaly',
        'transform': lambda y: np.where(y == 'Ya (Anomali)', 1, 0),
        'class_names': ['Normal Risk Assessment', 'Risk Score Spike Anomaly'],
        'is_binary': True
    },
