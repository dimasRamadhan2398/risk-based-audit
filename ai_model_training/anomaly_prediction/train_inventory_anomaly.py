...
dataset_path = os.path.join(current_dir, 'inventory_data.csv')
print(f"Loading Inventory dataset from: {dataset_path}")
...
# ======================================================================
print("\n======================================================================")
print(" 1. INVENTORY DATA PREPROCESSING & FEATURE ENGINEERING ")
print("======================================================================")

# Step 1: Strip surrounding spaces
text_columns = [col for col in ['Entitas Gudang', 'Kategori Barang', 'Deskripsi Penyesuaian', 'TARGET: is_anomaly'] if col in df.columns]
df[text_columns] = df[text_columns].apply(lambda col: col.astype("string").str.strip())

# Step 2: Convert numeric columns
numeric_cols = [
    'adjustment_amount (Juta Rp)', 'stock_variance_pct', 
    'adjustment_frequency_month', 'has_supporting_doc (1=Ya, 0=Tidak)', 
    'approver_role_level (1-4)', 'is_manual_override (1=Ya, 0=Tidak)'
]
df[numeric_cols] = df[numeric_cols].apply(pd.to_numeric, errors='coerce')

# Step 3: Deduplication
duplicate_cols = [col for col in df.columns if col != 'ID Penyesuaian']
...
# Step 4: Advanced Inventory Domain Feature Engineering
print("Applying Domain-Specific Inventory & Asset Feature Engineering...")
df['log_adjustment_amount'] = np.log1p(df['adjustment_amount (Juta Rp)'])
df['amount_variance_composite'] = df['adjustment_amount (Juta Rp)'] * df['stock_variance_pct']
df['risk_composite_score'] = ((1 - df['has_supporting_doc (1=Ya, 0=Tidak)']) * 3.0) + \
                             (df['is_manual_override (1=Ya, 0=Tidak)'] * 2.5) + \
                             (df['adjustment_frequency_month'] * 0.4) + \
                             (5 - df['approver_role_level (1-4)'])

# Input Feature Columns List
feature_cols = [
    'Entitas Gudang', 'Kategori Barang', 'Deskripsi Penyesuaian',
    'adjustment_amount (Juta Rp)', 'stock_variance_pct', 
    'adjustment_frequency_month', 'has_supporting_doc (1=Ya, 0=Tidak)', 
    'approver_role_level (1-4)', 'is_manual_override (1=Ya, 0=Tidak)',
    'log_adjustment_amount', 'amount_variance_composite', 'risk_composite_score'
]

X_raw = df[feature_cols].values
print(f"Selected {len(feature_cols)} clean inventory input features.")

# One-Hot Encoding for categorical columns (0: Entitas Gudang, 1: Kategori Barang, 2: Deskripsi Penyesuaian)
...
targets_config = {
    'Inventory Anomaly Detection (is_anomaly)': {
        'column': 'TARGET: is_anomaly',
        'transform': lambda y: np.where(y == 'Ya (Anomali)', 1, 0),
        'class_names': ['Normal Inventory', 'Anomaly Inventory'],
        'is_binary': True
    },
