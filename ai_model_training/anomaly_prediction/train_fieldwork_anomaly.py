...
dataset_path = os.path.join(current_dir, 'fieldwork_data.csv')
print(f"Loading Fieldwork dataset from: {dataset_path}")
...
# ======================================================================
print("\n======================================================================")
print(" 1. FIELDWORK DATA PREPROCESSING & FEATURE ENGINEERING ")
print("======================================================================")

# Step 1: Strip surrounding spaces
text_columns = [col for col in ['Entitas Auditee', 'Jenis Audit', 'Deskripsi Fieldwork', 'TARGET: is_anomaly'] if col in df.columns]
df[text_columns] = df[text_columns].apply(lambda col: col.astype("string").str.strip())

# Step 2: Convert numeric columns
numeric_cols = [
    'completion_duration_days', 'historical_avg_duration_days', 
    'sampling_coverage_pct', 'audit_findings_count', 
    'duration_zscore', 'is_instant_approval (1=Ya, 0=Tidak)'
]
df[numeric_cols] = df[numeric_cols].apply(pd.to_numeric, errors='coerce')

# Step 3: Deduplication
duplicate_cols = [col for col in df.columns if col != 'ID Penugasan']
...
# Step 4: Advanced Fieldwork Domain Feature Engineering
print("Applying Domain-Specific Audit Fieldwork Feature Engineering...")
df['duration_ratio'] = df['completion_duration_days'] / (df['historical_avg_duration_days'] + 1e-5)
df['findings_per_day'] = df['audit_findings_count'] / (df['completion_duration_days'] + 0.1)
df['risk_composite_score'] = (df['is_instant_approval (1=Ya, 0=Tidak)'] * 3.0) + \
                             (np.abs(df['duration_zscore']) * 1.5) + \
                             ((1.0 - (df['sampling_coverage_pct'] / 100.0)) * 2.0)

# Input Feature Columns List
feature_cols = [
    'Entitas Auditee', 'Jenis Audit', 'Deskripsi Fieldwork',
    'completion_duration_days', 'historical_avg_duration_days', 
    'sampling_coverage_pct', 'audit_findings_count', 
    'duration_zscore', 'is_instant_approval (1=Ya, 0=Tidak)',
    'duration_ratio', 'findings_per_day', 'risk_composite_score'
]

X_raw = df[feature_cols].values
print(f"Selected {len(feature_cols)} clean fieldwork input features.")

# One-Hot Encoding for categorical columns (0: Entitas Auditee, 1: Jenis Audit, 2: Deskripsi Fieldwork)
...
targets_config = {
    'Fieldwork Anomaly Detection (is_anomaly)': {
        'column': 'TARGET: is_anomaly',
        'transform': lambda y: np.where(y == 'Ya (Anomali)', 1, 0),
        'class_names': ['Normal Fieldwork', 'Anomaly Fieldwork'],
        'is_binary': True
    },
