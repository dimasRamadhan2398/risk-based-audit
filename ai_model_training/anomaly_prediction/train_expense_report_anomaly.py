...
dataset_path = os.path.join(current_dir, 'expense_report_data.csv')
print(f"Loading Expense Report dataset from: {dataset_path}")
...
# ======================================================================
print("\n======================================================================")
print(" 1. EXPENSE REPORT DATA PREPROCESSING & FEATURE ENGINEERING ")
print("======================================================================")

# Step 1: Strip surrounding spaces
text_columns = [col for col in ['Entitas', 'Kategori Pengeluaran', 'Deskripsi Pengeluaran', 'TARGET: is_anomaly'] if col in df.columns]
df[text_columns] = df[text_columns].apply(lambda col: col.astype("string").str.strip())

# Step 2: Convert numeric columns
numeric_cols = [
    'claim_amount (Juta Rp)', 'is_sequential_invoice (1=Ya, 0=Tidak)', 
    'is_exact_round_amount (1=Ya, 0=Tidak)', 'duplicate_claim_count', 
    'days_since_last_claim', 'is_over_limit_threshold (1=Ya, 0=Tidak)'
]
df[numeric_cols] = df[numeric_cols].apply(pd.to_numeric, errors='coerce')

# Step 3: Deduplication
duplicate_cols = [col for col in df.columns if col != 'ID Klaim']
...
# Step 4: Advanced Expense Domain Feature Engineering
print("Applying Domain-Specific Financial Expense Feature Engineering...")
df['log_claim_amount'] = np.log1p(df['claim_amount (Juta Rp)'])
df['amount_per_duplicate'] = df['claim_amount (Juta Rp)'] * (df['duplicate_claim_count'] + 1)
df['risk_composite_score'] = (df['is_sequential_invoice (1=Ya, 0=Tidak)'] * 2.5) + \
                             (df['is_over_limit_threshold (1=Ya, 0=Tidak)'] * 3.0) + \
                             (df['duplicate_claim_count'] * 0.5)

# Input Feature Columns List
feature_cols = [
    'Entitas', 'Kategori Pengeluaran', 'Deskripsi Pengeluaran',
    'claim_amount (Juta Rp)', 'is_sequential_invoice (1=Ya, 0=Tidak)', 
    'is_exact_round_amount (1=Ya, 0=Tidak)', 'duplicate_claim_count', 
    'days_since_last_claim', 'is_over_limit_threshold (1=Ya, 0=Tidak)',
    'log_claim_amount', 'amount_per_duplicate', 'risk_composite_score'
]

X_raw = df[feature_cols].values
print(f"Selected {len(feature_cols)} clean expense input features.")

# One-Hot Encoding for categorical columns (0: Entitas, 1: Kategori Pengeluaran, 2: Deskripsi Pengeluaran)
...
targets_config = {
    'Expense Anomaly Detection (is_anomaly)': {
        'column': 'TARGET: is_anomaly',
        'transform': lambda y: np.where(y == 'Ya (Anomali)', 1, 0),
        'class_names': ['Normal Expense', 'Anomaly Expense'],
        'is_binary': True
    },
