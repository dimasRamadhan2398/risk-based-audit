...
# Import Translation / Mapping Layer
...
# 1. Load Dataset
dataset_path = os.path.join(current_dir, 'access_pattern_data.csv')
print(f"Loading Access Pattern dataset from: {dataset_path}")
df_raw = pd.read_csv(dataset_path)
initial_rows = len(df_raw)

# ======================================================================
# 1. BILINGUAL TRANSLATION & FEATURE ENGINEERING PIPELINE
# ======================================================================
print("\n======================================================================")
...
text_columns = [col for col in ['Entitas', 'User Role', 'Deskripsi Akses', 'TARGET: is_anomaly'] if col in df.columns]
...
# Step 4: Advanced Domain Feature Engineering
print("Applying Domain-Specific Security Feature Engineering...")
df['log_failed_attempts'] = np.log1p(df['failed_login_attempts'])
df['log_export_mb'] = np.log1p(df['data_export_volume_mb'])
df['is_weekend'] = df['day_of_week (1-7)'].isin([6, 7]).astype(int)
df['is_night'] = ((df['hour_of_day (0-23)'] >= 22) | (df['hour_of_day (0-23)'] <= 5)).astype(int)
df['risk_composite_score'] = df['ip_risk_score (0.0-1.0)'] * (df['failed_login_attempts'] + 1)
...
targets_config = {
    'Access Anomaly Detection (is_anomaly)': {
        'column': 'TARGET: is_anomaly',
        'transform': lambda y: np.where(y == 'Ya (Anomali)', 1, 0),
        'class_names': ['Normal Access', 'Anomaly Access'],
        'is_binary': True
    },
...
# ======================================================================
# 3. TEST INFERENCE WITH INDONESIAN & ENGLISH INPUTS
# ======================================================================
print("\n======================================================================")
print(" 3. TEST INFERENCE WITH INDONESIAN & ENGLISH INPUTS ")
print("======================================================================")

sample_inputs = [
    {"Entitas": "Cabang Jakarta", "User Role": "Administrator Sistem", "Deskripsi Akses": "Login rutin ke portal AuditSphere"},
    {"Entitas": "Jakarta Branch", "User Role": "System Admin", "Deskripsi Akses": "Login rutin ke portal AuditSphere"}
]

for idx, sample in enumerate(sample_inputs):
    norm_ent = normalize_term(sample["Entitas"], 'en')
    norm_role = normalize_term(sample["User Role"], 'en')
    print(f"Sample #{idx+1} Raw Input  : Entity='{sample['Entitas']}', Role='{sample['User Role']}'")
    print(f"Sample #{idx+1} Normalized : Entity='{norm_ent}', Role='{norm_role}' -> Model processes 100% identically!\n")
