...
dataset_path = os.path.join(current_dir, 'treasury_data.csv')
print("=" * 80)
print(f" TRAINING & BENCHMARKING TREASURY ANOMALY PREDICTION ")
...
# Numeric conversions
num_raw_cols = ['Nilai Transaksi (Juta Rp)', 'Jam Transaksi (0-23)', 'Hari Transaksi (1-7)', 'Deviasi Kurs / Yield (%)']
...
print("\n[STEP 2] Applying Banking Treasury & Market Risk Feature Engineering...")
...
df['log_dev_rate'] = np.log1p(df['Deviasi Kurs / Yield (%)'])

# Treasury Market Risk & Governance indicators
...
df['is_offmarket_rate'] = (df['Deviasi Kurs / Yield (%)'] > 1.5).astype(int)
df['is_dealer_breach'] = (df['Status Limit Dealer'] == 'Melampaui Limit Dealer').astype(int)
df['is_cp_breach'] = (df['Status Limit Counterparty'] == 'Melampaui Limit Counterparty').astype(int)
df['is_unconfirmed_deal'] = (df['Status Konfirmasi Deal'].isin(['Mismatched Deal', 'Unconfirmed Deal Slip'])).astype(int)
df['is_midoffice_override'] = (df['Verifikasi Middle Office'] == 'Tanpa Verifikasi / Override Dealer').astype(int)
df['is_pdn_risk'] = (df['Indikasi Batas PDN'] == 'Potensi Melanggar Batas PDN').astype(int)

# Composite Treasury Risk Score
df['composite_treasury_risk'] = (
    df['is_offmarket_rate'] * 5.0 +
    df['is_dealer_breach'] * 4.5 +
    df['is_cp_breach'] * 4.0 +
    df['is_unconfirmed_deal'] * 3.5 +
    df['is_pdn_risk'] * 5.0 +
    df['is_night'] * 2.0 +
    df['is_midoffice_override'] * 3.0
)

# Categorical & Numeric feature lists
cat_cols = [
    'Entitas', 'Produk', 'Tipe Transaksi', 'Status Limit Dealer',
    'Status Limit Counterparty', 'Status Konfirmasi Deal',
    'Verifikasi Middle Office', 'Indikasi Batas PDN'
]
num_cols = [
    'Nilai Transaksi (Juta Rp)', 'Jam Transaksi (0-23)', 'Hari Transaksi (1-7)', 'Deviasi Kurs / Yield (%)',
    'hour_sin', 'hour_cos', 'day_sin', 'day_cos',
    'log_amount', 'log_dev_rate',
    'is_night', 'is_weekend', 'is_offmarket_rate', 'is_dealer_breach',
    'is_cp_breach', 'is_unconfirmed_deal', 'is_midoffice_override',
    'is_pdn_risk', 'composite_treasury_risk'
]
...
targets_config = {
    'Treasury Anomaly Detection (is_anomaly)': {
        'column': 'TARGET: is_anomaly',
        'transform': lambda y: np.where(y == 'Ya (Anomali)', 1, 0),
        'class_names': ['Normal Deal', 'Anomaly (Market Threat)'],
        'is_binary': True
    },
...
sample_cases = [
    {
        "desc": "Normal Government Bonds Placement (ID vs EN)",
        "id": {"Entitas": "Kantor Pusat", "Produk": "SBN / Government Bonds", "Tipe": "Buy / Purchase", "Nilai": 25000.0},
        "en": {"Entitas": "Head Office", "Produk": "SBN / Government Bonds", "Tipe": "Buy / Purchase", "Nilai": 25000.0}
    },
    {
        "desc": "Off-Market Foreign Exchange Deal (ID vs EN)",
        "id": {"Entitas": "Cabang Singapura", "Produk": "FX Forward / Swap", "Tipe": "Sell / Placement", "Nilai": 45000.0},
        "en": {"Entitas": "Singapore Branch", "Produk": "FX Forward / Swap", "Tipe": "Sell / Placement", "Nilai": 45000.0}
    }
]
...
print("\n" + "=" * 80)
print(" [SUCCESS] TREASURY ANOMALY PREDICTION PIPELINE EXECUTED SUCCESSFULLY! ")
print("=" * 80)
