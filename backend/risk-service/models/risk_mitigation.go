...
type WeeklyMonitoringCheck struct {
	ID        string    `json:"id"`
...
type MonitoringCheck struct {
	...
	StartDate time.Time               `json:"startDate"`
	EndDate   time.Time               `json:"endDate"`
	Weeks     []WeeklyMonitoringCheck `json:"weeks,omitempty"`
}
...
func GenerateMonitoringChecks(start, end time.Time) []MonitoringCheck {
	...
	for !current.After(end) {
		...
		var weeks []WeeklyMonitoringCheck
		wCurrent := mStart
		wIndex := 1
		for !wCurrent.After(mEnd) {
			wEnd := wCurrent.AddDate(0, 0, 7).Add(-time.Second)
			if wEnd.After(mEnd) {
				wEnd = mEnd
			}

			weeks = append(weeks, WeeklyMonitoringCheck{
				ID:        fmt.Sprintf("M%dW%d", i, wIndex),
				Label:     fmt.Sprintf("Minggu %d (%s - %s)", wIndex, wCurrent.Format("02 Jan"), wEnd.Format("02 Jan")),
				Checked:   false,
				Notes:     "",
				StartDate: wCurrent,
				EndDate:   wEnd,
