...
# ── Step 1: Package ──────────────────────────────────────────────────────────
echo "📦 [1/4] Packaging backend source for dev..."
tar \
  --exclude='.git' \
  --exclude='*/vendor' \
  --exclude='*.tar.gz' \
...
echo "🔧 [4/4] Extracting and starting backend (DEV) on VPS..."
ssh -o ServerAliveInterval=15 -o ServerAliveCountMax=6 -o StrictHostKeyChecking=no "$VPS_USER@$VPS_IP" bash <<EOF
  ...
  echo "  → Building and starting development containers (Kong Port 8081)..."
