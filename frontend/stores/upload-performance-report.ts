...
export const useUploadPerformanceReportStore = defineStore('upload-performance-report', () => {
  ...
  const uploadReport = async (payload: {
    ...
    try {
      ...
      toast.showError(error.data?.message || 'Failed to upload performance report document.');
      throw error;
    } finally {
      loading.value = false;
    }
  ...
  const deleteReport = async (id: string, currentPeriod?: string, currentYear?: number) => {
    ...
    try {
      ...
      toast.showError(error.data?.message || 'Failed to delete performance report document.');
      throw error;
    } finally {
      loading.value = false;
    }
