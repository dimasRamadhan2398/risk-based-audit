<template>
  <div class="p-6 max-w-full mx-auto space-y-6 min-h-screen min-w-full">
    ...
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('assignmentLetter.upload.title') }}</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('assignmentLetter.upload.subtitle') }}</p>
      </div>
    </div>
        ...
        <UCard :ui="{ body: 'p-6' }" class="shadow-sm border border-gray-200 dark:border-gray-800">
          <template #header>
            <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
              ...
              {{ t('assignmentLetter.upload.formTitle') }}
            </h3>
          </template>
          ...
          <form @submit.prevent="handleUpload" class="space-y-6">
            ...
            <UFormField :label="t('assignmentLetter.upload.description')">
              <UTextarea 
                ...
                :placeholder="t('assignmentLetter.upload.descriptionPlaceholder')" 
                class="w-full"
              />
            </UFormField>

            <div class="space-y-2 pt-2">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('assignmentLetter.upload.fileLabel') }}</label>
              <div 
              ...
              >
                ...
                <div v-if="!form.fileName" class="space-y-3">
                  ...
                  <div>
                    <p class="text-sm text-gray-600 dark:text-gray-300 font-semibold">{{ t('assignmentLetter.upload.dropzonePrompt') }}</p>
                    <p class="text-md text-gray-400 mt-1">{{ t('assignmentLetter.upload.dropzoneHint') }}</p>
                  </div>
                </div>
                ...
                <div v-else class="space-y-3">
                  ...
                  >
                    {{ t('assignmentLetter.upload.removeFile') }}
                  </button>
                </div>
              </div>
            </div>
            ...
            <UButton 
              type="submit" 
              :label="t('assignmentLetter.upload.submitButton')" 
              color="primary" 
        ...
        <UCard :ui="{ body: 'p-4' }" class="shadow-sm border border-gray-200 dark:border-gray-800 h-full">
          <template #header>
            <div class="flex justify-between items-center">
              <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                {{ t('assignmentLetter.upload.tableTitle') }}
              </h3>
              <UBadge color="primary" variant="subtle">
                {{ t('assignmentLetter.upload.documentsCount', { count: store.uploadedDocuments.length }) }}
              </UBadge>
            </div>
          </template>
          ...
          <TableEntities
            ...
            :empty-state="{
              ...
              label: t('assignmentLetter.upload.emptyTitle'),
              description: t('assignmentLetter.upload.emptyDesc')
            }"
          >
...
const columns = computed(() => [
  { accessorKey: 'title', header: t('assignmentLetter.upload.columns.title'), class: 'w-[50%]' },
  { accessorKey: 'fileName', header: t('assignmentLetter.upload.columns.file'), class: 'w-[24%]' },
  { accessorKey: 'created_at', header: t('assignmentLetter.upload.columns.date'), class: 'w-[16%]' },
  { accessorKey: 'actions', header: t('assignmentLetter.upload.columns.actions'), class: 'w-[10%]' }
])
...
const processFile = (file: File) => {
  if (file.size > 10 * 1024 * 1024) {
    alert(t('assignmentLetter.upload.fileSizeLimit'))
    return
  }
