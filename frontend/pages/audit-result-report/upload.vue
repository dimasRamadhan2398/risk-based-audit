<template>
  <div class="p-6 max-w-full mx-auto space-y-6 min-h-screen min-w-full">
    ...
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('auditResultReport.upload.title') }}</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('auditResultReport.upload.subtitle') }}</p>
      </div>
    </div>
        ...
        <UCard :ui="{ body: 'p-6' }" class="shadow-sm border border-gray-200 dark:border-gray-800">
          <template #header>
            <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
              ...
              {{ t('auditResultReport.upload.formTitle') }}
            </h3>
          </template>

          <form @submit.prevent="handleUpload" class="space-y-6">
            <UFormField :label="t('auditResultReport.upload.documentTitle')" required>
              <UInput 
                v-model="form.title" 
                :placeholder="t('auditResultReport.upload.documentTitlePlaceholder')" 
                class="w-full"
                required
              />
            </UFormField>

            <UFormField :label="t('auditResultReport.upload.description')">
              <UTextarea 
                v-model="form.description" 
                :placeholder="t('auditResultReport.upload.descriptionPlaceholder')" 
                class="w-full"
              />
            </UFormField>

            <div class="space-y-2 pt-2">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('auditResultReport.upload.fileLabel') }}</label>
              <div 
              ...
              >
                ...
                <div v-if="!form.fileName" class="space-y-3">
                  ...
                  <div>
                    <p class="text-sm text-gray-600 dark:text-gray-300 font-semibold">{{ t('auditResultReport.upload.dropzonePrompt') }}</p>
                    <p class="text-md text-gray-400 mt-1">{{ t('auditResultReport.upload.dropzoneHint') }}</p>
                  </div>
                </div>
                ...
                <div v-else class="space-y-3">
                  ...
                  >
                    {{ t('auditResultReport.upload.removeFile') }}
                  </button>
                </div>
              </div>
            </div>
            ...
            <UButton 
              type="submit" 
              :label="t('auditResultReport.upload.submitButton')" 
              color="primary" 
        ...
        <UCard :ui="{ body: 'p-4' }" class="shadow-sm border border-gray-200 dark:border-gray-800 h-full">
          <template #header>
            <div class="flex justify-between items-center">
              <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                {{ t('auditResultReport.upload.tableTitle') }}
              </h3>
              <UBadge color="primary" variant="subtle">
                {{ t('auditResultReport.upload.documentsCount', { count: store.uploadedDocuments.length }) }}
              </UBadge>
            </div>
          </template>
          ...
          <TableEntities
            ...
            :empty-state="{
              ...
              label: t('auditResultReport.upload.emptyTitle'),
              description: t('auditResultReport.upload.emptyDesc')
            }"
          >
...
>>>>>>> 19ed793 (fix: table display on every page)
                  :title="t('auditResultReport.upload.actions.view')" 
...
=======
                  ...
                  size="sm" 
>>>>>>> 19ed793 (fix: table display on every page)
                  :title="t('auditResultReport.upload.actions.download')" 
...
=======
                  size="sm" 
>>>>>>> 19ed793 (fix: table display on every page)
                  :title="t('auditResultReport.upload.actions.delete')" 
...
const columns = computed(() => [
  { accessorKey: 'title', header: t('auditResultReport.upload.columns.title'), class: 'w-[50%]' },
  { accessorKey: 'fileName', header: t('auditResultReport.upload.columns.file'), class: 'w-[24%]' },
  { accessorKey: 'created_at', header: t('auditResultReport.upload.columns.date'), class: 'w-[16%]' },
  { accessorKey: 'actions', header: t('auditResultReport.upload.columns.actions'), class: 'w-[10%]' }
])
...
const processFile = (file: File) => {
  if (file.size > 10 * 1024 * 1024) {
    alert(t('auditResultReport.upload.fileSizeLimit'))
    return
  }
...
<<<<<<< HEAD
  if (await useGlobalModalStore().confirmDelete({ description: t('auditResultReport.upload.deleteConfirm') })) {
=======
  if (confirm(t('auditResultReport.upload.deleteConfirm'))) {
>>>>>>> 19ed793 (fix: table display on every page)
