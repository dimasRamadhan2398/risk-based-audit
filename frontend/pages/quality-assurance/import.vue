<template>
  <div class="p-6 max-w-full mx-auto space-y-6 min-h-screen min-w-full">
    ...
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('qualityAssurance.importQar.title') }}</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('qualityAssurance.importQar.subtitle') }}</p>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-10">
      ...
      <div class="w-full space-y-6">
        <UCard :ui="{ body: 'p-6' }" class="shadow-sm border border-gray-200 dark:border-gray-800">
          ...
          <form @submit.prevent="handleUpload" class="space-y-6">
            <UFormField :label="t('qualityAssurance.importQar.documentTitle')" required>
              <UInput 
                v-model="form.title" 
            ...
            <UFormField :label="t('qualityAssurance.importQar.description')">
              <UTextarea 
                ...
                :placeholder="t('qualityAssurance.importQar.descriptionPlaceholder')" 
                class="w-full"
              />
            </UFormField>

            <div class="space-y-2 pt-2">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('qualityAssurance.importQar.fileLabel') }}</label>
              <div 
              ...
              >
                ...
                <div v-if="!form.fileName" class="space-y-3">
                  ...
                  <div>
                    <p class="text-sm text-gray-600 dark:text-gray-300 font-semibold">{{ t('qualityAssurance.importQar.dropzonePrompt') }}</p>
                    <p class="text-md text-gray-400 mt-1">{{ t('qualityAssurance.importQar.dropzoneHint') }}</p>
                  </div>
                </div>
                ...
                <div v-else class="space-y-3">
                  ...
                  >
                    {{ t('qualityAssurance.importQar.removeFile') }}
                  </button>
                </div>
              </div>
            </div>
      ...
      <div class="w-full">
        <UCard :ui="{ body: 'p-4' }" class="shadow-sm border border-gray-200 dark:border-gray-800 h-full">
          ...
          <TableEntities
            :data="store.qarImportedReports"
            :columns="columns"
            :loading="store.loading"
            :empty-state="{
              icon: 'i-lucide-folder-open',
              label: t('qualityAssurance.importQar.emptyTitle'),
              description: t('qualityAssurance.importQar.emptyDesc')
            }"
          >
...
const columns = computed(() => [
  { accessorKey: 'title', header: t('qualityAssurance.importQar.columns.title'), class: 'w-[50%]' },
  { accessorKey: 'fileName', header: t('qualityAssurance.importQar.columns.file'), class: 'w-[24%]' },
  { accessorKey: 'created_at', header: t('qualityAssurance.importQar.columns.date'), class: 'w-[16%]' },
  { accessorKey: 'actions', header: t('qualityAssurance.importQar.columns.actions'), class: 'w-[10%]' }
])
...
const processFile = (file: File) => {
  if (file.size > 10 * 1024 * 1024) {
    alert(t('qualityAssurance.importQar.fileSizeLimit'))
    return
  }
