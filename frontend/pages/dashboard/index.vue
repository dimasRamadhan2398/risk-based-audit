<template>
  <div class="p-6 md:p-8 space-y-8 min-h-screen">
    <!-- Header Section -->
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-1.5">
          Welcome back, {{ authStore.getUser?.fullName || 'Andi' }}. Here's your risk & audit performance overview.
        </p>
      </div>
      <div>
        <UButton
        ...
        >
          <template #leading>
            <UIcon name="i-lucide-rotate-cw" :class="{ 'animate-spin': isSyncing }" class="size-4" />
          </template>
          Sync
        </UButton>
      </div>
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      <!-- Card 1: Total Risks -->
      <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 flex flex-col justify-between h-[135px]">
        ...
        <div class="mt-2">
          <h3 class="text-3xl font-bold text-slate-900 tracking-tight">{{ totalRisks }}</h3>
          <div class="flex items-center gap-1 text-xs font-semibold text-emerald-500 mt-1">
            <span>↑ 12%</span>
      ...
      <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 flex flex-col justify-between h-[135px]">
        ...
        <div class="mt-2">
          <h3 class="text-3xl font-bold text-slate-900 tracking-tight">{{ auditPlansCount }}</h3>
      ...
      <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 flex flex-col justify-between h-[135px]">
        ...
        <div class="mt-2">
          <h3 class="text-3xl font-bold text-emerald-600 tracking-tight">{{ completedAuditsCount }}</h3>
          <div class="flex items-center gap-1 text-xs font-semibold text-emerald-500 mt-1">
            <span>Updated</span>
    ...
    <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 space-y-6">
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <!-- Planned Audits -->
        <div class="bg-slate-50 border border-slate-100 rounded-xl p-5 flex flex-col justify-between h-[120px]">
          <div class="flex justify-between items-start">
          ...
          <div class="mt-1">
            <h4 class="text-2xl font-bold text-slate-900">{{ auditMainStats.plannedAudit }}</h4>
            <p class="text-[11px] mt-1" :class="plannedAuditTrend.color">
              {{ plannedAuditTrend.icon }} {{ plannedAuditTrend.value }} <span class="text-slate-400 font-normal">from last month</span>
            </p>
          </div>
        </div>
        ...
        <div class="bg-slate-50 border border-slate-100 rounded-xl p-5 flex flex-col justify-between h-[120px]">
          ...
          <div class="mt-1">
            <h4 class="text-2xl font-bold text-slate-900">{{ auditMainStats.openFinding }}</h4>
            <p class="text-[11px] mt-1" :class="openFindingTrend.color">
              {{ openFindingTrend.icon }} {{ openFindingTrend.value }} <span class="text-slate-400 font-normal">from last month</span>
            </p>
          </div>
        </div>
        ...
        <div class="bg-slate-50 border border-slate-100 rounded-xl p-5 flex flex-col justify-between h-[120px]">
          ...
          <div class="mt-1">
            <h4 class="text-2xl font-bold text-slate-900">{{ (auditMainStats.executionStatus * 100).toFixed(0) }}%</h4>
            <p class="text-[11px] mt-1" :class="executionStatusTrend.color">
              {{ executionStatusTrend.icon }} {{ executionStatusTrend.value }} <span class="text-slate-400 font-normal">from last month</span>
            </p>
          </div>
        </div>
        ...
        <div class="bg-slate-50 border border-slate-100 rounded-xl p-5 flex flex-col justify-between h-[120px]">
          ...
          <div class="mt-1">
            <h4 class="text-2xl font-bold text-slate-900">{{ (auditMainStats.atrCompliance * 100).toFixed(0) }}%</h4>
            <p class="text-[11px] mt-1" :class="atrComplianceTrend.color">
              {{ atrComplianceTrend.icon }} {{ atrComplianceTrend.value }} <span class="text-slate-400 font-normal">from last month</span>
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div class="lg:col-span-8 bg-white border border-slate-100 shadow-sm rounded-2xl p-6 flex flex-col justify-between">
        <div>
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-bold text-slate-800">Inherent vs Residual Risk by Department</h3>
            <USelect v-model="activeYear" :options="yearlyFilters" size="sm" class="w-24 bg-white" />
          </div>
      ...
      <div class="lg:col-span-4 bg-white border border-slate-100 shadow-sm rounded-2xl p-6 flex flex-col justify-between">
        <div>
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-bold text-slate-800">Action Taken Report</h3>
            <UButton to="/action-taken-report" variant="ghost" color="neutral" size="sm">Details</UButton>
          </div>
          <div class="h-[220px] w-full relative flex items-center justify-center">
            ...
            >
              <div class="text-center flex flex-col items-center justify-center">
                <span class="text-xl font-bold text-slate-900 leading-tight">{{ atrStore.stats.donePercent }}%</span>
        ...
        <!-- Custom Legend -->
        <div class="flex flex-wrap justify-center gap-x-4 gap-y-2 mt-4 px-2 border-t border-slate-50 pt-4">
          <div class="flex items-center gap-1.5">
            <span class="w-2.5 h-2.5 rounded-full bg-[#4d00ff]"></span>
            <span class="text-[11px] font-semibold text-slate-600">Completed ({{ atrStore.stats.donePercent }}%)</span>
          </div>
          <div class="flex items-center gap-1.5">
            <span class="w-2.5 h-2.5 rounded-full bg-slate-400"></span>
            <span class="text-[11px] font-semibold text-slate-600">In Progress ({{ atrStore.stats.wipPercent }}%)</span>
          </div>
          <div class="flex items-center gap-1.5">
            <span class="w-2.5 h-2.5 rounded-full bg-[#ff5c02]"></span>
            <span class="text-[11px] font-semibold text-slate-600">Overdue ({{ atrStore.stats.latePercent }}%)</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 space-y-6">
      <div class="flex items-center justify-between">
        ...
        <UButton to="/risk-profile" variant="ghost" color="neutral" size="sm">Configure</UButton>
      </div>

      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <!-- Heat Map Grid on the left -->
        <div class="lg:col-span-5 flex flex-col items-center bg-slate-50/50 border border-slate-100 rounded-2xl p-6">
          <h4 class="text-sm font-semibold text-slate-700 mb-6 w-full text-left">Risk Heat Map</h4>
          <div class="flex flex-row gap-4 items-center">
            <!-- Y-axis label -->
            <div class="flex flex-col items-center">
              <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider origin-center -rotate-90 whitespace-nowrap">
                Probability
              </span>
            </div>
            <!-- Heat map grid -->
        ...
        <div class="lg:col-span-7 bg-white border border-slate-100 rounded-2xl p-6">
          ...
          <UTable
            ...
            :empty-state="{ icon: 'i-heroicons-circle-stack-20-solid', label: 'Belum ada data yang dimasukkan ke Risk Heat Map.' }"
      ...
      <div class="lg:col-span-4 bg-white border border-slate-100 shadow-sm rounded-2xl p-6 flex flex-col justify-between">
        <div>
          <div class="flex items-center justify-between mb-6">
            ...
            <UButton to="/annual-audit" variant="ghost" color="neutral" size="sm">Configure</UButton>
          </div>
          <div class="space-y-4">
            ...
            <UProgress :model-value="progressModel" color="secondary" class="h-2 rounded" />
          </div>
        </div>
        <div class="grid grid-cols-3 gap-2 border-t border-slate-50 pt-6 mt-6 text-center">
          <div>
            <p class="text-[11px] text-slate-400 font-semibold uppercase">Planned</p>
            <h4 class="text-lg font-bold text-slate-800 mt-1">{{ auditCoverage.plannedAudits }}</h4>
          </div>
          <div>
            <p class="text-[11px] text-slate-400 font-semibold uppercase">Completed</p>
            <h4 class="text-lg font-bold text-slate-800 mt-1">{{ auditCoverage.completedAudits }}</h4>
          </div>
          <div>
            <p class="text-[11px] text-slate-400 font-semibold uppercase">Remaining</p>
            <h4 class="text-lg font-bold text-slate-800 mt-1">{{ auditCoverage.remainingAudits }}</h4>
          </div>
        </div>
      </div>

      <!-- Action Taken Reports Table -->
      <div class="lg:col-span-8 bg-white border border-slate-100 shadow-sm rounded-2xl p-6">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-bold text-slate-800">Action Taken Reports (ATR)</h3>
          <UButton to="/action-taken-report" variant="ghost" color="neutral" size="sm">View Full Report</UButton>
        </div>
        <UTable
          :data="atrTableData"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 space-y-6">
        <div class="flex items-center justify-between">
          <h2 class="text-xl font-bold text-slate-900 tracking-tight">Audit Execution Status</h2>
          <UBadge color="primary" label="Q4 2025" variant="soft"></UBadge>
        </div>
        <div class="space-y-5">
          <div v-for="(item, index) in dashboardExecutionStatus" :key="index" class="space-y-2">
            <div class="flex justify-between items-center">
            ...
            <UProgress :model-value="item.percentage" color="secondary" class="h-1.5 rounded" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-bold text-slate-900 tracking-tight">Recent Finding Issues</h2>
        </div>
        <UTable
      ...
      <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 space-y-6">
        ...
        <div class="space-y-3">
          ...
          >
            ...
            <UBadge :color="risk.impact * risk.likelihood > 15 ? 'red' : 'warning'" variant="soft" size="sm" class="font-semibold">
      ...
      <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 space-y-6">
        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex items-center gap-3">
              <div
                class="rounded-lg bg-sky-50 w-8 h-8 flex items-center justify-center"
              >
            ...
            <UBadge color="info" variant="soft" size="sm" class="font-semibold">Scheduled</UBadge>
          </div>
          <div v-if="annualPlanStore.plans.length === 0" class="text-center py-6 text-slate-400 text-sm">
