<template>
  <UModal
    ...
    :ui="{
      width: 'sm:max-w-3xl',
      base: 'overflow-hidden rounded-2xl bg-white dark:bg-gray-900 shadow-2xl border border-gray-200 dark:border-gray-800'
    }"
  ...
  >
    <div class="flex flex-col max-h-[90vh]">
      ...
      <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-800 bg-gradient-to-r from-gray-50 to-white dark:from-gray-900 dark:to-gray-850 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400">
              Provision an isolated AuditSphere instance (e.g. <span class="font-mono text-primary font-medium">accenture.auditsphere.id</span>)
            </p>
          </div>
        </div>
        ...
        <button
          type="button"
          class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 p-1.5 rounded-lg transition-colors focus:outline-none"
          @click="closeModal"
        >
          <UIcon
            name="i-lucide-x"
      ...
      <div class="px-6 py-3 bg-gray-50/70 dark:bg-gray-800/40 border-b border-gray-100 dark:border-gray-800">
      ...
      <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-850 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <button
            type="button"
            :disabled="isProvisioning"
            class="px-4 py-2 rounded-xl text-xs font-semibold text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 transition-colors"
            @click="closeModal"
          >
            Cancel
          </button>
...
interface Props {
  modelValue?: boolean
  nextSuggestedPort?: number
  nextSuggestedKongPort?: number
}

const props = withDefaults(defineProps<Props>(), {
  modelValue: false,
  nextSuggestedPort: 3010,
  nextSuggestedKongPort: 8090
})

const emit = defineEmits<{
  'update:modelValue': [value: boolean]
  'site-created': [site: Record<string, unknown>]
}>()
...
const copyCommand = async () => {
  try {
    await navigator.clipboard.writeText(generatedCliCommand.value)
    isCopied.value = true
    setTimeout(() => {
